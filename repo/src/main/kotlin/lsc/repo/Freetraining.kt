...
enum class FreetrainingStateDbo {
...
data class FreetrainingDbo(
    ...
    val category: String,
    val date: LocalDate,
    val state: FreetrainingStateDbo,
    val planId: Int,
) {
    val isScheduled = state == FreetrainingStateDbo.Scheduled
    val isCheckedin = state == FreetrainingStateDbo.Checkedin
...
object FreetrainingDboTable : IntIdTable("FREETRAININGS", "ID") {
    ...
    val date = date("DATE")
    val state = enumerationByName<FreetrainingStateDbo>("STATE", 32)
...
object ExposedFreetrainingRepo : FreetrainingRepo {
    ...
    override fun selectAllScheduled(cityId: Int): List<FreetrainingDbo> = transaction {
        FreetrainingDboTable
            .join(VenueDboTable, JoinType.LEFT, onColumn = FreetrainingDboTable.venueId, otherColumn = VenueDboTable.id)
            .selectAll()
            .where { (VenueDboTable.cityId eq cityId) and (FreetrainingDboTable.state eq FreetrainingStateDbo.Scheduled) }
            .map {
                FreetrainingDbo.fromRow(it)
            }
    }
    ...
    override fun deleteNonCheckedinBefore(threshold: LocalDate): List<FreetrainingDbo> = transaction {
        val deleted = FreetrainingDboTable.selectAll().where {
            (FreetrainingDboTable.state neq FreetrainingStateDbo.Checkedin) and (FreetrainingDboTable.date less threshold)
