...
object ExposedVenueRepo : VenueRepo {
    ...
    private fun VenueDbo.Companion.fromRow(row: ResultRow) = VenueDbo(
        ...
        visitLimits = row[VenueDboTable.visitLimits]?.let { VisitLimitsDbo.fromSqlValue(it) },
        lastSync = row[VenueDboTable.lastSync],
        createdAt = row[VenueDboTable.dateCreated],
        deletedAt = row[VenueDboTable.dateDeleted],
    )
}

// TODO refector this
private fun VisitLimitsDbo.Companion.fromSqlValue(value: String) = value.split(",").map { it.toIntOrNull() }.let {
    VisitLimitsDbo(it[0], it[1], it[2], it[3])
}

private fun VisitLimitsDbo.toSqlValue() = listOf(small, medium, large, xlarge).joinToString(",")

public data class VisitLimitsDbo(
    val small: Int?,
    val medium: Int?,
    val large: Int?,
    val xlarge: Int?,
