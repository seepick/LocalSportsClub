...
public enum class ActivityStateDbo(val label: String) {
...
public data class ActivityDbo(
    ...
    val state: ActivityStateDbo, // updateable
    val teacher: String?, // updateable
    val description: String?, // updateable
) {
    val isBooked = state == ActivityStateDbo.Booked
    val isCheckedin = state == ActivityStateDbo.Checkedin
...
internal object ActivityDboTable : IntIdTable("ACTIVITIES", "ID") {
    ...
    val state = enumerationByName<ActivityStateDbo>("STATE", 32)
...
object ExposedActivityRepo : ActivityRepo {
    ...
    override fun selectAllBooked(cityId: Int): List<ActivityDbo> = transaction {
        ActivityDboTable
            .join(VenueDboTable, JoinType.LEFT, onColumn = ActivityDboTable.venueId, otherColumn = VenueDboTable.id)
            .selectAll()
            .where { (VenueDboTable.cityId eq cityId) and (ActivityDboTable.state eq ActivityStateDbo.Booked) }
    ...
    override fun selectNewestCheckedinDate(): LocalDate? = transaction {
        ActivityDboTable.select(ActivityDboTable.from).where { ActivityDboTable.state.eq(ActivityStateDbo.Checkedin) }
    ...
    override fun deleteBlanksBefore(threshold: LocalDate): List<ActivityDbo> = transaction {
        ...
        val deletedActivities = ActivityDboTable.selectAll().where {
            (ActivityDboTable.state eq ActivityStateDbo.Blank) and (ActivityDboTable.from less thresholdDateTime)
