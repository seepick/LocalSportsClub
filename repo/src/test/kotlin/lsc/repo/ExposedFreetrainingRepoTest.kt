    ...
    init {
        ...
        describe("selectAllScheduled") {
            it("Given non-matching Then empty") {
                insertTrainingAndVenue(cityId = cityId) { copy(state = FreetrainingStateDbo.Blank) }

                freetrainingRepo.selectAllScheduled(cityId).shouldBeEmpty()
            }
            it("Given matching Then return it") {
                insertTrainingAndVenue(cityId = cityId) { copy(state = FreetrainingStateDbo.Scheduled) }
        ...
        describe("deleteNonCheckedinBefore") {
            ...
            it("Given newer non-checkedin Then keep") {
                insertTrainingAndVenue(cityId = cityId) { copy(date = todayDate, state = FreetrainingStateDbo.Blank) }
