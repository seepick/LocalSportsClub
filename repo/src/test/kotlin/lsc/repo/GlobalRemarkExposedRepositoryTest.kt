...
class GlobalRemarkExposedRepositoryTest : DescribeSpec({
    ...
    val remarkType = Arb.enum<GlobalRemarkTypeDbo>().next()
    val remarkType1 = GlobalRemarkTypeDbo.Activity
    val remarkType2 = GlobalRemarkTypeDbo.Teacher
...
fun Arb.Companion.globalRemarkDbo() = arbitrary {
    GlobalRemarkDbo(
        id = int().bind(),
        type = enum<GlobalRemarkTypeDbo>().bind(),
