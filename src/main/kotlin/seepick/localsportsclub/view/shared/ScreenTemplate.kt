                        ...
                        VenueDetail(
                            ...
                            visitsModel = viewModel.userPlan?.let { plan ->
                                selectedVenue!!.visitLimits?.let { limits ->
                                    limits.forPlan(plan)?.let { planLimit ->
                                        selectedVenue!!.toMonthlyVisitsModel(clock.today(), planLimit, limits)
                                    }
                                }
                            },
                            ...
                            activityRemarksCount = selectedVenue!!.activityRemarks.size,
                            teacherRemarksCount = selectedVenue!!.teacherRemarks.size,
                            onViewActivityRemarks = viewModel::onViewActivityRemarks,
                            onViewTeacherRemarks = viewModel::onViewTeacherRemarks,
                            showLinkedVenues = viewModel.showLinkedVenues,
                            isSyncing = mainViewModel.isSyncInProgress,
                            onSyncVenueActivities = viewModel::onSyncVenueActivities,
                            isSyncVenueActivitiesInProgress = viewModel.isSyncVenueActivitiesInProgress,
                            onSyncVenueDetails = viewModel::onSyncVenueDetails,
                            isSyncVenueDetailsInProgress = viewModel.isSyncVenueDetailsInProgress,
